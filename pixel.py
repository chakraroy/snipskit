#!/usr/bin/python

import apa102
import time
import threading
import paho.mqtt.client as mqtt
import json

try:
    import queue as Queue
except ImportError:
    import Queue as Queue

class Pixels:
    PIXELS_N = 3

    def __init__(self):
        self.basis = [0] * 3 * self.PIXELS_N
        self.basis[0] = 1
        self.basis[4] = 1
        self.basis[8] = 2

        self.colors = [0] * 3 * self.PIXELS_N
        self.dev = apa102.APA102(num_led=self.PIXELS_N)

        self.next = threading.Event()
        self.queue = Queue.Queue()
        self.thread = threading.Thread(target=self._run)
        self.thread.daemon = True
        self.thread.start()

    def wakeup(self, direction=0):
        def f():
            self._wakeup(direction)

        self.next.set()
        self.queue.put(f)

    def listen(self):
        self.next.set()
        self.queue.put(self._listen)

    def think(self):
        self.next.set()
        self.queue.put(self._think)

    def speak(self):
        self.next.set()
        self.queue.put(self._speak)

    def off(self):
        self.next.set()
        self.queue.put(self._off)

    def _run(self):
        while True:
            func = self.queue.get()
            func()

    def _wakeup(self, direction=0):
        for i in range(1, 25):
            colors = [i * v for v in self.basis]
            self.write(colors)
            time.sleep(0.01)

        self.colors = colors

    def _listen(self):
        for i in range(1, 25):
            colors = [i * v for v in self.basis]
            self.write(colors)
            time.sleep(0.01)

        self.colors = colors

    def _think(self):
        colors = self.colors

        self.next.clear()
        while not self.next.is_set():
            colors = colors[3:] + colors[:3]
            self.write(colors)
            time.sleep(0.2)

        t = 0.1
        for i in range(0, 5):
            colors = colors[3:] + colors[:3]
            self.write([(v * (4 - i) / 4) for v in colors])
            time.sleep(t)
            t /= 2

        # time.sleep(0.5)

        self.colors = colors

    def _speak(self):
        colors = self.colors

        self.next.clear()
        while not self.next.is_set():
            for i in range(5, 25):
                self.write([(v * i / 24) for v in colors])
                time.sleep(0.01)

            time.sleep(0.3)

            for i in range(24, 4, -1):
                self.write([(v * i / 24) for v in colors])
                time.sleep(0.01)

            time.sleep(0.3)

        self._off()

    def _off(self):
        self.write([0] * 3 * self.PIXELS_N)

    def write(self, colors):
        for i in range(self.PIXELS_N):
            self.dev.set_pixel(i, int(colors[3*i]), int(colors[3*i + 1]), int(colors[3*i + 2]))

        self.dev.show()


pixels = Pixels()


CLIENT_ID = "zero"

mqtt_client = mqtt.Client()

CURRENT_SESSION_ID = "."

def on_connect(client, userdata, flags, rc):
    #mqtt_client.subscribe('hhermes/dialogueManager/sessionStarted')
    #mqtt_client.subscribe('hermes/dialogueManager/sessionEnded')
    mqtt_client.subscribe('hermes/asr/startListening')
    mqtt_client.subscribe('hermes/tts/say')
    mqtt_client.subscribe('hermes/nlu/query')
    mqtt_client.subscribe('hermes/hotword/toggleOn')
    mqtt_client.subscribe('hermes/nlu/partialQuery')
    mqtt_client.subscribe('hermes/asr/textCaptured')

# elif (msg.topic == 'hermes/nlu/query') or (msg.topic == 'hermes/nlu/partialQuery'):
def on_message(client, userdata, msg):
    data = json.loads(msg.payload)
    global CURRENT_SESSION_ID

    if msg.topic == 'hermes/asr/startListening':
        print("mqtt startListneing Called")
        if data['siteId'] == CLIENT_ID:
            CURRENT_SESSION_ID = data['sessionId']
            print("start listening.." + CURRENT_SESSION_ID)
            pixels.listen()
    elif msg.topic == 'hermes/asr/textCaptured':
        print("mqtt textCaptured Called")
        #if data['sessionId'] == CURRENT_SESSION_ID:
        if data['siteId'] == CLIENT_ID:
            print("pixels think")
            pixels.think()
    elif msg.topic == 'hermes/tts/say':
        print("mqtt say Called")
        if data['siteId'] == CLIENT_ID:
            print("saying something")
            pixels.speak()
    elif msg.topic == 'hermes/hotword/toggleOn':
        print("mqtt hotword on Called")
        if data['siteId'] == CLIENT_ID:
            CURRENT_SESSION_ID = "."
            print("Stopping everything.. Pixels off!")
            pixels.off()


if __name__ == '__main__':
    pixels.off()
    time.sleep(1)
    
    mqtt_client.on_connect = on_connect
    mqtt_client.on_message = on_message
    mqtt_client.connect('10.0.1.22', 1883)
    mqtt_client.loop_forever()

    
